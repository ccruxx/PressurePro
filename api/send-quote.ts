  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f3f4f6;padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:0 40px 24px;">
            <p style="margin:0 0 12px;color:#111827;font-size:16px;font-weight:600;">Job Details</p>
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
              <tr style="background:#f9fafb;">
                <td style="padding:10px 16px;color:#6b7280;font-size:14px;width:40%;">Service</td>
                <td style="padding:10px 16px;color:#374151;font-size:14px;font-weight:500;">${quote.service}</td>
              </tr>
              <tr>
                <td style="padding:10px 16px;color:#6b7280;font-size:14px;border-top:1px solid #f0f0f0;">Measurement</td>
                <td style="padding:10px 16px;color:#374151;font-size:14px;font-weight:500;border-top:1px solid #f0f0f0;">${quote.measurement}</td>
              </tr>
              <tr style="background:#f9fafb;">
                <td style="padding:10px 16px;color:#6b7280;font-size:14px;border-top:1px solid #f0f0f0;">Dirt Level</td>
                <td style="padding:10px 16px;color:#374151;font-size:14px;font-weight:500;border-top:1px solid #f0f0f0;">${quote.dirtLevel}</td>
              </tr>
              ${
                quote.addons.length > 0
                  ? `<tr>
                <td style="padding:10px 16px;color:#6b7280;font-size:14px;border-top:1px solid #f0f0f0;">Add-ons</td>
                <td style="padding:10px 16px;color:#374151;font-size:14px;font-weight:500;border-top:1px solid #f0f0f0;">${quote.addons.join(", ")}</td>
              </tr>`
                  : ""
              }
            </table>
...
<body style="margin:0;padding:24px;font-family:Arial,sans-serif;background:#f9fafb;">
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
    <tr><td style="background:#1e7fc1;padding:20px 28px;">
      ...
      <p style="margin:4px 0 0;color:rgba(255,255,255,0.8);font-size:14px;">${formatCurrency(quote.priceLow)} – ${formatCurrency(quote.priceHigh)} estimated</p>
    </td></tr>
    <tr><td style="padding:24px 28px;">
      ...
      <h3 style="margin:0 0 12px;color:#111827;font-size:16px;">Quote Details</h3>
      <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:6px;overflow:hidden;margin-bottom:16px;">
        <tr style="background:#f9fafb;"><td style="padding:8px 12px;color:#6b7280;font-size:14px;">Service</td><td style="padding:8px 12px;">${quote.service}</td></tr>
        <tr><td style="padding:8px 12px;color:#6b7280;font-size:14px;border-top:1px solid #f0f0f0;">Measurement</td><td style="padding:8px 12px;border-top:1px solid #f0f0f0;">${quote.measurement}</td></tr>
        <tr style="background:#f9fafb;"><td style="padding:8px 12px;color:#6b7280;font-size:14px;border-top:1px solid #f0f0f0;">Dirt Level</td><td style="padding:8px 12px;border-top:1px solid #f0f0f0;">${quote.dirtLevel}</td></tr>
        ${quote.addons.length > 0 ? `<tr><td style="padding:8px 12px;color:#6b7280;font-size:14px;border-top:1px solid #f0f0f0;">Add-ons</td><td style="padding:8px 12px;border-top:1px solid #f0f0f0;">${quote.addons.join(", ")}</td></tr>` : ""}
      </table>
    ...
    <tr><td style="background:#f3f4f6;padding:14px 28px;text-align:center;">
      <p style="margin:0;color:#9ca3af;font-size:12px;">Submitted via dfwpristinepowerwashing.com/quote-preview</p>
    </td></tr>
  </table>
</body>
</html>`;
  ...
  try {
    const [customerResult, ownerResult] = await Promise.all([
      resend.emails.send({
        ...
        subject: `Your Instant Quote – ${payload.quote.service}`,
        html: buildCustomerEmail(payload),
      }),
      resend.emails.send({
        from: `Quote Calculator <${FROM_EMAIL}>`,
        to: OWNER_EMAIL,
        subject: `New Quote Lead: ${payload.customer.name} – ${payload.quote.service} (${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(payload.quote.priceLow)}–${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(payload.quote.priceHigh)})`,
