...
function buildCustomerEmail(payload: QuotePayload): string {
  ...
  const addonsRow =
    quote.addons.length > 0
      ? `<tr>
          <td colspan="2" style="padding:10px 16px;color:#6b7280;font-size:14px;border-top:1px solid #f0f0f0;">
  ...
  try {
    const [customerResult, ownerResult] = await Promise.all([
      ...
      resend.emails.send({
        ...
        to: OWNER_EMAIL,
        subject: `New Quote Lead: ${payload.customer.name} – ${serviceTitle} (${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(payload.quote.priceLow)}–${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(payload.quote.priceHigh)})`,
